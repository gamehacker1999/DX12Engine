...
float4 main(VertexToPixel input) : SV_TARGET
{
    ...
    float3 velocity = velocityBuffer.SampleLevel(basicSampler, input.uv, 0);
    float2 pixelSize = float2(1.0 / 1280, 1.0 / 720); //Need to pass this later
	...
	//sample velocity uv  
    float2 vel = velocityBuffer.Sample(basicSampler, input.uv).xy;
    float2 historyUV = input.uv + vel;
	
	
    float4 history = ConvertToYCoCg(previousFrame.Sample(basicSampler, historyUV));
