...
Texture2D directLightingTransparency : register(t3);
...
float4 main(VertexToPixel input) : SV_TARGET
{
    ...
    float3 directColorTransparency = directLightingTransparency.Sample(basicSampler, input.uv);

    float3 finalCol = directColor + indDiffuse + indSpec;
    
    return float4(directColorTransparency, 1.0f);
}