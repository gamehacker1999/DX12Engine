...
bool ImGui::SelectableInput(const char* str_id, bool selected, ImGuiSelectableFlags flags, std::string& buf)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    if (temp_input_is_active || temp_input_start)
    {
        ...
        ret = ImGui::TempInputText(window->DC.LastItemRect, id, "##Input", name, 10000000, ImGuiInputTextFlags_None);
        window->DC.CursorPos = pos_after;
    }
    else
    {
        window->DrawList->AddText(pos_before, ImGui::GetColorU32(ImGuiCol_Text), (char*)buf.c_str());
    }
