...
void GetPrimitiveProperties(inout float4 texColor, inout float3 position, inout float3 normal, inout float3 metalColor, inout float roughness, Attributes attrib)
{
    ...
    texColor = material[index + 0].SampleLevel(basicSampler, uv, 0);
